...
- Decide the SMTP page's fate. It currently stores transport settings and sends a test message only — nothing sends mail when an event or failure occurs. Either build a real alerting stage (global SMTP with per-site propagation and event-driven alert rules, which needs its own ADR because a shared credential cuts against site isolation) or leave SMTP permanently transport-only.
